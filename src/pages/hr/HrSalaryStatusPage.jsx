  ...
  return (
    <Modal open={open} onClose={onClose} title="Import bảng chấm công" size="sm">
      <div className="space-y-4 py-2">
        <div>
          <label className="block text-xs font-semibold text-muted mb-1.5">Tháng chấm công</label>
          <select
            ...
            className="w-full px-3 py-2.5 rounded-xl border border-hairline-2 text-sm bg-surface">
            {options.map(o => (
        ...
        <div className="text-[11px] text-muted bg-canvas rounded-xl px-3.5 py-3 space-y-1.5">
          <p className="font-bold text-ink text-xs">File "BẢNG CHI TIẾT CHẤM CÔNG"</p>
          ...
          <p className="text-amber-700 dark:text-amber-300">⚠ Tải lại cùng tháng sẽ <strong>ghi đè</strong> dữ liệu cũ
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                { label: 'NV xưởng',      value: result.factoryEmployees ?? 0, color: 'text-gold' },
                { label: 'Đã khớp',       value: result.matched ?? 0, color: 'text-emerald-600 dark:text-emerald-300' },
              ].map(s => (
                <div key={s.label} className="bg-canvas rounded-xl px-3 py-2.5 text-center">
                  <p className="text-[10px] text-muted font-semibold">{s.label}</p>
        ...
        {!uploading ? (
          <label className="flex items-center justify-center gap-2 px-5 py-3 rounded-xl
            bg-gold text-white text-sm font-bold cursor-pointer
        ...
        action={
          <div className="flex gap-2">
            <button onClick={() => setAttendanceOpen(true)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-surface border border-hairline-2 text-ink hover:bg-canvas transition-colors">
            ...
            <button onClick={handleExport} disabled={exporting}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-gold text-white hover:bg-gold-deep transition-colors disabled:opacity-60">
        ...
        {['', 'PENDING', 'APPROVED', 'REJECTED'].map(s => (
          <button key={s} onClick={() => { setStatusFilter(s); setPage(0); }}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors
              ${statusFilter === s ? 'bg-chrome text-white' : 'bg-surface border border-hairline-2 text-muted hover:bg-canvas'}`}>
              ...
              {rows.map(s => {
                ...
                return (
                  <Tr key={s.id}>
                    ...
                    <Td>
                      {s.rejectReason
                        ? <span className="text-xs text-red-600 dark:text-red-300">{s.rejectReason}</span>
                        : '—'}
                    </Td>
