...
import {
  TrendingUp, ShoppingBag, CheckCircle, Truck, Wallet, AlertCircle,
...
function StatCard({ label, icon: Icon, accent, value, isPrice, loading, alert, onClick }) {
  const map = {
    ...
    blue:   { bg: 'bg-blue-50',      ico: 'text-blue-500',    val: 'text-[#1C1C1E]' },
    red:    { bg: 'bg-red-50',       ico: 'text-red-500',     val: 'text-red-500' },
    orange: { bg: 'bg-orange-50',    ico: 'text-orange-500',  val: 'text-orange-500' },
...
function TopProductRow({ p, i, imgUrl, pct }) {
  ...
  return (
    <div className="flex items-center gap-2 sm:gap-3">
      <span className={`text-xs font-bold w-4 sm:w-5 text-right shrink-0
        ${i===0?'text-[#C9A84C]':i===1?'text-[#8E8878]':i===2?'text-amber-700':'text-[#C4B9A8]'}`}>
        {i + 1}
      </span>
      <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-[#F0EBE3] overflow-hidden shrink-0">
        {imgUrl
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <span className="text-[9px] sm:text-[10px] text-[#8E8878] shrink-0 tabular-nums">
            {parseFloat(animQty.toFixed(3)).toLocaleString('vi-VN')} sp
          </span>
        </div>
      </div>
    </div>
...
export default function AccountantDashboardPage() {
  ...
  const load = useCallback(async (r) => {
    ...
    const groupBy = groupByFromRange(r);
    try {
      const [s, c, tp, ts, tc, ds] = await Promise.all([
        accountantDashboardApi.getSummary(r.from, r.to),
        accountantDashboardApi.getChart(r.from, r.to, groupBy),
        ...
        accountantDashboardApi.getDebtStats(r.from, r.to).catch(() => null),
      ]);
      setSummary(s); setChartData(c || []);
      setTopProducts(tp || []); setTopSellers(ts || []);
      setTopCustomers(tc || []); setDebtStats(ds);
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4 sm:space-y-5 min-h-full bg-[#FAF7F2]">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <StatCard label="Tổng đơn hàng"  icon={ShoppingBag} accent="gold"  value={summary?.totalOrders}   loading={loading} />
        <StatCard label="Đã thanh toán"  icon={Wallet}      accent="green" value={summary?.totalPaid}     loading={loading} isPrice />
        <StatCard label="Công nợ"        icon={TrendingUp}  accent="red"   value={summary?.totalDebt}     loading={loading} isPrice />
          ...
          <div style={{ userSelect: 'none', WebkitUserSelect: 'none' }}>
            <ResponsiveContainer width="100%" height={240}>
              <ComposedChart data={chartData}
                ...
                barCategoryGap="30%" barGap={2} onClick={() => {}}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-5">
        ...
        {/* Top products */}
        <div className="bg-white rounded-2xl p-4 sm:p-5 border border-[#F0EBE3] shadow-sm">
          <h2 className="text-xs sm:text-sm font-semibold text-[#1C1C1E] mb-3 sm:mb-4">Món bán chạy nhất</h2>
          {loading ? (
