...
// 6. OrderCard: thêm onCancel prop
// 7. Desktop table row: thêm onCancel
// 8. Mobile OrderCard: thêm onCancel
...
function PaymentMethodCell({ value, onSave, disabled }) {
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(o => !o)} className="inline-flex items-center gap-1 group" title="Click để thay đổi phương thức thanh toán">
...
// ── PartialPaymentModal — giữ nguyên từ original ──────────────────────────────
function PartialPaymentModal({ order, onClose, onConfirm, loading }) {
  ...
  const handleConfirm = () => {
    ...
    if (paymentMethod === 'BANK_TRANSFER' && !bankName.trim()) { setError('Vui lòng nhập tên ngân hàng'); return; }
    onConfirm({ paidAmount: paidNum, debtDays: hasDeadline ? Number(deadlineDays) : 0, paymentMethod,
      bankName: isBankTransfer ? bankName.trim() : undefined, transactionRef: isBankTransfer ? transactionRef.trim() : undefined });
  };
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
            ...
            {paidNum > 0 && paidNum <= remaining && (
              <div className={`flex flex-col gap-1.5 px-3 py-2 rounded-lg text-xs ${isFullPayment ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
                <div className="flex items-center gap-1.5">
                  {isFullPayment ? <><CheckCircle size={12} /> Thanh toán đủ — đơn sẽ chuyển sang <strong>Hoàn thành</strong></> : <><AlertCircle size={12} /> Còn lại: <strong>{formatPrice(newRemaining)}</strong> — đơn sẽ là <strong>Còn nợ</strong></>}
                </div>
                ...
                {canWaive && waiveConfirm && (
                  <div className="flex flex-col gap-1.5 p-2 bg-amber-100 rounded-lg border border-amber-300">
                    ...
                    <p className="text-[10px] text-amber-700">Đơn sẽ được đánh dấu <strong>Hoàn thành + Đã thanh toán</strong> luôn.</p>
                    <div className="flex gap-1.5">
                      ...
                      <button onClick={() => onConfirm({ paidAmount: paidNum, debtDays: 0, paymentMethod, bankName: isBankTransfer ? bankName.trim() : undefined, transactionRef: isBankTransfer ? transactionRef.trim() : undefined, waiveRemainder: true, actualPaid: totalWouldPay })}
                        className="flex-1 py-1 rounded-lg bg-amber-600 text-white text-[10px] font-semibold hover:bg-amber-700">Xác nhận bỏ số lẻ</button>
                    </div>
                  </div>
                )}
              </div>
          ...
          {isBankTransfer && (
            <div className="space-y-3 p-3 bg-blue-50 rounded-xl border border-blue-100">
              ...
              <div><label className="block text-[10px] font-medium text-blue-600 mb-1">Tên ngân hàng *</label>
                <input value={bankName} onChange={e => setBankName(e.target.value)} placeholder="VD: Vietcombank, BIDV..." className="w-full px-3 py-2 text-sm rounded-xl border border-blue-200 bg-white focus:outline-none focus:border-blue-400" /></div>
              <div><label className="block text-[10px] font-medium text-blue-600 mb-1">Mã giao dịch / Số tham chiếu</label>
                <input value={transactionRef} onChange={e => setTransactionRef(e.target.value)} placeholder="VD: FT23161234567" className="w-full px-3 py-2 text-sm rounded-xl border border-blue-200 bg-white focus:outline-none focus:border-blue-400" /></div>
            </div>
          )}
            ...
            <div className="space-y-2">
              ...
              {hasDeadline && (
                <div className="flex items-center gap-2 pl-11">
                  ...
                  <input type="number" min="1" max="365" value={deadlineDays} onChange={e => { setDeadlineDays(e.target.value); setError(''); }} placeholder="Số ngày"
...
// ── StatusActionButtons — THÊM onCancel + nút Hủy đơn ────────────────────────
function StatusActionButtons({ order, onPendingPayment, onComplete, onPartialPayment, onCancel, loading }) {
  ...
  return (
    <div className="flex items-center gap-1.5 flex-wrap">
      ...
      {canCancel && (
        <button onClick={e => { e.stopPropagation(); onCancel(); }} disabled={loading} title="Hủy đơn hàng"
...
function InvoiceButton({ order, invoiceLoadingId, onInvoice }) {
  ...
  return (
    <button onClick={e => { e.stopPropagation(); onInvoice(order.id, e); }} disabled={!!invoiceLoadingId}
      title={isThisLoading?'Đang tạo hoá đơn...':isOtherLoading?'Chờ đơn khác xong':'Xem hoá đơn PDF'}
...
// ── OrderCard mobile — THÊM onCancel ─────────────────────────────────────────
    ...
    <div className={`rounded-2xl border p-4 space-y-3 transition-all ${isThisInvoice?'bg-[#C9A84C]/5 border-[#C9A84C]/40':'bg-white border-[#F0EBE3]'} ${isActioning?'opacity-60':''}`}>
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="text-right shrink-0">
          ...
          {o.paidAmount>0&&<p className="text-[10px] text-emerald-600 font-medium">Đã thu: {formatPrice(o.paidAmount)}</p>}
      ...
      {isThisInvoice&&<div className="flex items-center gap-2 bg-[#C9A84C]/10 rounded-lg px-3 py-2"><BtnSpinner size={11} colorClass="border-[#C9A84C] !border-t-transparent" /><span className="text-[11px] font-medium text-[#C9A84C]">Đang tạo hoá đơn PDF...</span></div>}
    </div>
  );
}
...
export default function AccountantOrdersPage() {
  ...
  const handleInvoice = async (orderId, e) => {
    ...
    } catch { toast('Không thể tải hoá đơn','error'); }
  ...
  const handleComplete = async (orderId) => {
    ...
    try {
      ...
      if (res?.data?.success===false){toast(res.data.message||'Không thể hoàn thành','error');return;}
    ...
    } catch(e){ toast(e?.response?.data?.message||'Lỗi khi hoàn thành','error'); }
    finally { setActionLoading(null); }
  };
  ...
  const handlePartialPayment = async (payload) => {
    ...
    } catch(e){ toast(e?.response?.data?.message||e?.message||'Lỗi ghi nhận thanh toán','error'); }
  ...
  const handleReceiptFileChange = async (e) => {
    ...
    } catch { toast('Lỗi upload phiếu nhận','error'); }
  ...
  const executeBulkComplete = async (mode, isFinalPartial) => {
    ...
    try {
      ...
      else if (successCount===0) toast(`❌ Không cập nhật được đơn nào. ${errors.length} lỗi`,'error');
      else toast(`⚠️ ${successCount} đơn thành công, ${errors.length} đơn lỗi`,'warning');
      setSelectedIds(new Set()); fetchOrders(page);
    } catch { toast('Lỗi bulk update','error'); }
      ...
      <div className="flex-shrink-0 px-4 sm:px-6 py-4 bg-white border-b border-[#F0EBE3]">
        <div className="flex items-center gap-2 mb-3">
          ...
          <div className="relative hidden sm:block">
            ...
            <input type="text" placeholder="Tìm đơn, khách hàng..." value={searchInput} onChange={e=>setSearchInput(e.target.value)}
          ...
          <div className="hidden sm:flex items-center gap-1.5 flex-wrap">
            <DateRangePicker from={dateRange.from} to={dateRange.to} onChange={r=>{setDateRange(r);setPage(0);}} placeholder="Khoảng ngày" />
            {productFilter&&<button onClick={()=>setProductFilter('')} className="flex items-center gap-1 px-2 py-1 bg-[#C9A84C]/10 text-[#C9A84C] rounded-lg text-xs font-medium">Bỏ lọc món <X size={10}/></button>}
            {customerFilter&&<button onClick={()=>setCustomerFilter('')} className="flex items-center gap-1 px-2 py-1 bg-sky-50 text-sky-600 rounded-lg text-xs font-medium">Bỏ lọc KH <X size={10}/></button>}
          </div>
          <div className="flex sm:hidden items-center gap-1.5">
            <div className="relative"><Search size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#8E8878]"/><input type="text" placeholder="Tìm..." value={searchInput} onChange={e=>setSearchInput(e.target.value)} className="border border-[#E8DDD0] rounded-xl pl-8 pr-3 py-2 text-sm bg-white focus:outline-none focus:border-[#C9A84C] w-28"/></div>
          ...
          <button onClick={handleExport} disabled={exporting} className="p-2 rounded-xl bg-emerald-50 text-emerald-600 hover:bg-emerald-100 transition-colors disabled:opacity-60 shrink-0" title="Xuất Excel">
        ...
        <div className="flex gap-1.5 overflow-x-auto pb-0.5 scrollbar-hide">
          {FILTER_TABS.map(t=>(
            <button key={t.value} onClick={()=>setStatusFilter(t.value)}
              className={`shrink-0 px-2.5 sm:px-3 py-1.5 rounded-lg text-[10px] sm:text-xs font-medium transition-colors ${statusFilter===t.value?'bg-[#C9A84C] text-white':'bg-[#F0EBE3] text-[#8E8878] hover:bg-[#E8DDD0]'}`}>
              {t.label}
            </button>
          ))}
        </div>
                        ...
                        <tr key={o.id} className={`border-b border-[#F0EBE3] last:border-0 transition-colors ${getRowBg(o)} ${isThisInvoice?'opacity-80':''} ${isActioning?'opacity-60':''}`}>
                          ...
                          <td className="px-4 py-3">
                            ...
                            {o.status==='COMPLETED'&&o.paymentStatus==='PAID'?(
                              ...
                              {paidAmount>0&&<p className="text-[10px] text-emerald-600 font-medium whitespace-nowrap">Đã thu: {formatPrice(paidAmount)}</p>}
                              {paidAmount>0&&paidAmount<Number(o.finalAmount)&&<p className="text-[10px] text-orange-500 font-medium whitespace-nowrap">Còn: {formatPrice(Number(o.finalAmount)-paidAmount)}</p>}
                            </>)}
                          </td>
                          ...
                          <td className="px-4 py-3">
                            {o.receiptFileUrl?(
                              ...
                              <span className="text-[10px] text-[#C4B9A8]">Không có</span>
                            ):(
      ...
      {selectedIds.size>0&&(()=>{
        ...
        return (
          <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40 flex items-center gap-2 bg-white border border-[#E8DDD0] rounded-2xl shadow-xl px-4 py-2.5">
            <div className="flex flex-col mr-2"><span className="text-xs font-semibold text-[#5C5C5C]">{selectedIds.size} đơn đã chọn</span><span className="text-[11px] font-bold text-[#C9A84C]">{formatP(totalSelected)}</span></div>
            <button onClick={()=>setBulkConfirm({mode:'DELIVERED_UNPAID',isFinalPartial:false})} className="px-3 py-1.5 rounded-lg bg-orange-50 text-orange-700 border border-orange-200 text-xs font-semibold hover:bg-orange-100">🚚 Đã giao hàng</button>
            <button onClick={()=>setBulkConfirm({mode:'DELIVERED_PAID',isFinalPartial:false})} className="px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-200 text-xs font-semibold hover:bg-emerald-100">✅ Hoàn thành (đã TT)</button>
        ...
        return (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4">
              ...
              <div className="bg-[#FAF7F2] rounded-xl px-4 py-3 flex justify-between items-center"><span className="text-xs text-[#8E8878]">{selectedIds.size} đơn — Tổng giá trị</span><span className="text-sm font-bold text-[#C9A84C]">{formatP(totalSelected)}</span></div>
              {bulkConfirm.mode==='DELIVERED_PAID'&&<p className="text-xs text-[#8E8878]">Các đơn đã chọn sẽ được đánh dấu Hoàn thành và Đã thanh toán đủ.</p>}
              <div className="flex gap-2 pt-2">
                ...
                <button disabled={bulkLoading} onClick={async()=>{const c=bulkConfirm;setBulkConfirm(null);await executeBulkComplete(c.mode,false);}} className="flex-1 py-2 rounded-xl bg-[#C9A84C] text-white text-sm font-semibold disabled:opacity-50">
