  ...
  return (
    <Modal open onClose={onClose} title={`Nhập giá vốn — ${lot.ingredientName}`} size="md">
      <div className="space-y-4">
        ...
        <div className="bg-canvas border border-hairline rounded-xl p-3 grid grid-cols-2 gap-x-3 gap-y-2 text-sm">
        ...
        {/* Chọn cách nhập giá */}
        <div>
          <p className="text-sm font-semibold text-ink mb-2">Cách nhập giá</p>
        ...
        <div>
          ...
          <div className="relative">
            ...
            />
            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-muted">đ</span>
          </div>
        </div>

        {/* Kết quả tính */}
        {unitCost != null && (
          <div className="bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/28 rounded-xl p-3 space-y-1.5">
            <div className="flex justify-between text-sm">
              <span className="text-emerald-700 dark:text-emerald-300">Giá vốn / {lot.unit || 'đvt'}</span>
              <span className="font-bold text-emerald-800 dark:text-emerald-300 tabular-nums">{fmtMoney(unitCost)} đ</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-emerald-700 dark:text-emerald-300">Giá vốn cả lô</span>
              <span className="font-semibold text-emerald-800 dark:text-emerald-300 tabular-nums">{fmtMoney(lotCost)} đ</span>
            </div>
            <p className="text-[10.5px] text-emerald-700 dark:text-emerald-300 pt-1 border-t border-emerald-200 dark:border-emerald-500/28">
...
function LotCard({ lot, onPrice }) {
  ...
  return (
    <div className="bg-surface rounded-2xl border border-hairline shadow-sm p-4 hover:shadow-md transition flex flex-col gap-3">
      {/* Header */}
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="font-bold text-ink text-base truncate">{lot.ingredientName}</p>
          <p className="flex items-center gap-1.5 text-xs text-muted mt-0.5 truncate">
        ...
        <span className={`flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-full font-semibold flex-shrink-0 ${
          priced ? 'bg-green-100 dark:bg-green-500/18 text-green-700 dark:text-green-300' : 'bg-orange-100 dark:bg-orange-500/18 text-orange-700 dark:text-orange-300'
        }`}>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="bg-canvas rounded-xl px-3 py-2">
          <p className="text-[10.5px] uppercase tracking-wide text-muted font-semibold">Số lượng</p>
          <p className="text-sm font-bold text-ink tabular-nums">
            {fmtQty(lot.quantity)} <span className="font-medium text-muted">{lot.unit}</span>
          </p>
        </div>
        <div className="bg-canvas rounded-xl px-3 py-2">
          <p className="text-[10.5px] uppercase tracking-wide text-muted font-semibold">Hạn sử dụng</p>
          <p className="text-sm font-bold text-ink flex items-center gap-1">
            <CalendarClock size={12} className="text-muted" /> {fmtDate(lot.expiryDate)}
          </p>
        </div>
      </div>
      ...
      <p className="flex items-center gap-1.5 text-xs text-muted">
        ...
        Người thao tác: <span className="text-ink font-medium">{lot.requestedByName || '—'}</span>
        {lot.receiptCode && <span className="font-mono text-gold">· {lot.receiptCode}</span>}
      </p>

      {priced ? (
        <div className="bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/28 rounded-xl px-3 py-2 text-sm">
          <div className="flex justify-between">
            <span className="text-emerald-700 dark:text-emerald-300">Giá vốn / {lot.unit}</span>
            <span className="font-bold text-emerald-800 dark:text-emerald-300 tabular-nums">{fmtMoney(lot.unitCost)} đ</span>
          </div>
          <p className="text-[10.5px] text-emerald-700 dark:text-emerald-300 mt-1">
            {lot.pricedByName || '—'} · {fmtTime(lot.pricedAt)}
          </p>
        </div>
      ) : (
        <button onClick={() => onPrice(lot)}
          className="w-full flex items-center justify-center gap-1.5 px-4 py-2.5 bg-gold text-white rounded-xl text-sm font-semibold hover:bg-gold-strong transition">
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-5">
      ...
      <div className="flex items-start sm:items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3">
          <Layers size={24} className="text-gold flex-shrink-0" />
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-ink">Điều chỉnh lô</h1>
        ...
        {[
          ...
          <button key={tb.key} onClick={() => setTab(tb.key)}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold border transition ${
              tab === tb.key
                ? 'bg-gold text-white border-gold'
                : 'bg-surface border-hairline-2 text-ink-2 hover:bg-canvas'
            }`}>
