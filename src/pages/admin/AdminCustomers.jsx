...
import {
  ...
  Building2, User as UserIcon, CalendarDays,
} from 'lucide-react';
...
export default function AdminCustomers() {
  ...
  }, [page, debouncedQ, filters.type, filters.isActive]);

  useEffect(() => { load(); }, [load]);
  useEffect(() => { setSelectedIds(new Set()); }, [page, filters.type, filters.isActive, debouncedQ]);
  ...
  const openDebtDays = (c, e) => {
    e.stopPropagation();
    setDebtDaysTarget(c);
    setDebtDaysValue(c.debtDays || 0);
    setDebtDaysOpen(true);
  };
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-black/5 p-3 sm:p-4 shadow-sm flex flex-col sm:flex-row gap-3">
        ...
        <select value={filters.type} onChange={e => { setFilters({ ...filters, type: e.target.value }); setPage(0); }} className={`${inputCls} sm:w-40`}>
          <option value="">Tất cả loại</option>
          <option value="COMPANY">Doanh nghiệp</option>
          <option value="RETAIL">Khách lẻ</option>
        </select>
        <select value={filters.isActive} onChange={e => { setFilters({ ...filters, isActive: e.target.value }); setPage(0); }} className={`${inputCls} sm:w-40`}>
                  ...
                  {data.content.map(c => {
                    ...
                    return (
                      <tr key={c.id}
                        ...
                        <td className="px-4 py-3">
                          <Badge className={c.customerType === 'COMPANY' ? 'bg-blue-50 text-blue-700 ring-blue-200' : 'bg-amber-50 text-amber-700 ring-amber-200'}>
                        ...
                        <td className="px-4 py-3 text-right" onClick={e => openDebtDays(c, e)}>
                          <span className={`text-xs font-semibold cursor-pointer hover:underline
                            ${c.debtDays > 0 ? 'text-orange-600' : 'text-[#C4B9A8]'}`}>
                ...
                return (
                  <div key={c.id}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className={`w-9 h-9 rounded-full flex items-center justify-center text-white shrink-0 ${c.customerType === 'COMPANY' ? 'bg-blue-500' : 'bg-[#C9A84C]'}`}>
                        {c.customerType === 'COMPANY' ? <Building2 size={15} /> : <UserIcon size={15} />}
                      </div>
      ...
      <Modal open={debtDaysOpen} onClose={() => !saving && setDebtDaysOpen(false)}
        ...
        footer={
          <div className="flex justify-end gap-2">
            <SecondaryButton onClick={() => setDebtDaysOpen(false)} disabled={saving}>Hủy</SecondaryButton>
            <PrimaryButton onClick={saveDebtDays} loading={saving}>Áp dụng</PrimaryButton>
          </div>
        }>
        {debtDaysTarget && (
          <p className="text-sm text-[#1C1C1E] mb-3">
            Khách: <span className="font-semibold">
              {debtDaysTarget.customerType === 'COMPANY' ? debtDaysTarget.companyName : debtDaysTarget.name}
            </span>
          </p>
        )}
        <Field label="Số ngày được phép công nợ" required>
          <input
            ...
            value={debtDaysValue}
            onChange={e => setDebtDaysValue(e.target.value)}
            className={inputCls}
            placeholder="0"
          />
        </Field>
        <p className="text-xs text-[#8E8878] mt-1.5">
          Từ 1–365 ngày. Đặt 0 để tắt công nợ — khách phải thanh toán ngay khi nhận hàng.
        </p>
      </Modal>
