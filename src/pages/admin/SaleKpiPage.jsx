    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-black/5 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-black/5 flex items-center gap-2">
          ...
          <h2 className="font-bold text-[#1C1C1E]">Doanh số theo Danh mục</h2>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[620px]">
              <thead className="bg-[#FAF7F2]">
                <tr className="text-[11px] uppercase tracking-wider text-[#8E8878]">
              ...
              <tbody>
                {categories.map((c, i) => (
                  <tr key={c.id ?? c.name} className="border-t border-black/5 hover:bg-[#FAF7F2]/40">
                    <td className="px-5 py-3">
                      {i < 3
                        ? <span className="text-lg">{['🥇', '🥈', '🥉'][i]}</span>
                        : <span className="text-sm text-[#8E8878] font-medium">{i + 1}</span>}
                    </td>
                    <td className="px-5 py-3 font-semibold text-[#1C1C1E]">{c.name}</td>
                    <td className="px-5 py-3 text-right text-[#1C1C1E]">
                      {c.quantity}{c.unit ? ` ${c.unit}` : ''}
                    </td>
                    <td className="px-5 py-3 text-right font-bold text-[#1C1C1E]">{fmt(c.amount)}</td>
                    <td className="px-5 py-3 text-center">
                      <button type="button" onClick={() => setCatDetail(c)}
                        className="text-xs font-semibold text-[#C9A84C] hover:text-[#B69842]">
                        Chi tiết
                      </button>
                    </td>
                  </tr>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {(catDetail.products || []).map((p, i) => (
                  <tr key={p.id ?? p.name} className="border-b border-black/5 last:border-0">
                    <td className="px-3 py-2.5 text-[#8E8878]">{i + 1}</td>
                    <td className="px-3 py-2.5 text-[#1C1C1E]">{p.name}</td>
                    <td className="px-3 py-2.5 text-right">
                      {p.quantity}{p.unit ? ` ${p.unit}` : ''}
                    </td>
                    <td className="px-3 py-2.5 text-right font-semibold text-[#1C1C1E]">{fmt(p.amount)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
