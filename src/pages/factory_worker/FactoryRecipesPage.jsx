        ...
        <ol className="space-y-1">
          {(r.steps || []).map((s, idx) => (
            <li key={s.id} className="flex items-center gap-2 text-xs text-[#1C1C1E] bg-[#FAF7F2] rounded-lg px-2.5 py-1.5">
              ...
              {s.requiresQc && <ShieldCheck size={12} className="text-amber-500 flex-shrink-0" title="Có kiểm soát" />}
...
function RecipeFormModal({ recipe, products, materials, stepTemplates, machines, onClose, onSaved, onStepTemplateCreated }) {
  ...
  const save = async () => {
    if (!form.factoryProductId || !form.name.trim() || !form.standardOutputQty) {
      setErr('Vui lòng điền đầy đủ: thành phẩm, tên biến thể, sản lượng chuẩn'); return;
    }
    if (form.items.some(it => !it.factoryMaterialId || !it.standardQty)) {
      setErr('Vui lòng điền đầy đủ thông tin nguyên liệu (hoặc xoá dòng trống)'); return;
    }
  ...
  return (
    <Modal open title={recipe ? 'Sửa biến thể sản xuất' : 'Tạo biến thể sản xuất mới'} onClose={onClose} size="xl"
      footer={
        <div className="flex justify-end gap-2">
          <SecondaryButton onClick={onClose} disabled={saving}>Huỷ</SecondaryButton>
          <PrimaryButton onClick={save} loading={saving}>Lưu biến thể</PrimaryButton>
        </div>
      }>
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Sản lượng thành phẩm chuẩn" required>
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            {form.items.length === 0 && (
              <p className="text-xs text-[#8E8878] italic text-center py-4">Chưa có nguyên liệu nào — nhấn "Thêm nguyên liệu" để bắt đầu</p>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-2">
                ...
                <div className="flex gap-2 items-start">
                  ...
                  <div className="flex-1 grid grid-cols-1 sm:grid-cols-4 gap-2">
                    <div className="sm:col-span-2">
                      ...
                      <input className={inputCls} value={step.stepName}
                    ...
                    <div>
                      ...
                      <input type="number" min="1" className={inputCls} value={step.durationMinutes}
                    ...
                    <div>
                      ...
                      <select className={inputCls} value={step.machineId}
                    ...
                    <div className="sm:col-span-4 flex items-center gap-2">
                      <label className="flex items-center gap-1.5 text-xs font-medium text-[#1C1C1E] cursor-pointer">
                        <input type="checkbox" checked={!!step.requiresQc}
                          onChange={e => setStep(idx, 'requiresQc', e.target.checked)}
                          className="rounded border-black/20" />
                        <ShieldCheck size={13} className="text-amber-500" /> Có kiểm soát (QC)
                      </label>
                    </div>
                  </div>
            ...
            {form.steps.length === 0 && (
              <p className="text-xs text-[#8E8878] italic text-center py-4">Chưa có bước nào — chọn mẫu bước phía trên hoặc tạo mẫu mới</p>
            )}
          </div>
        </div>
      </div>
