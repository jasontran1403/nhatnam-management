    ...
    <div className="bg-surface rounded-2xl border border-hairline shadow-sm overflow-hidden">
      <button onClick={() => setOpen(o => !o)}
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-ink">Quản lý nhà cung cấp</span>
        </div>
      ...
      {open && (
        <div className="px-4 pb-4 border-t border-hairline pt-3 space-y-3">
          <div className="flex items-center gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-2 text-sm rounded-xl border border-line focus:outline-none focus:border-gold bg-canvas"
              />
            </div>
          ...
          {loading ? (
            <div className="space-y-2">{[1, 2].map(i => <div key={i} className="h-12 bg-canvas rounded-xl animate-pulse" />)}</div>
          ) : filtered.length === 0 ? (
            ...
            <div className="max-h-72 overflow-y-auto divide-y divide-hairline -mx-1">
              {filtered.map(v => (
                <div key={v.id} className="flex items-center justify-between px-1 py-2.5">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted mt-0.5 flex items-center gap-2 flex-wrap">
                      <span className="bg-surface-2 px-1.5 py-0.5 rounded-full">{VENDOR_TYPE_LABELS[v.vendorType] || v.vendorType || 'Khác'}</span>
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-5">
      ...
      <div className="bg-gradient-to-r from-amber-50 dark:from-amber-500/10 to-amber-50/40 rounded-2xl p-4 flex items-center justify-between border border-amber-100 dark:border-amber-500/18">
        <div>
          ...
          <p className="text-2xl font-bold text-amber-700 dark:text-amber-300 mt-0.5">{formatCurrency(totalDebt)}</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {vendors.map(v => {
            ...
            return (
              <div key={v.vendorId}
                className="w-full bg-surface rounded-2xl border border-hairline shadow-sm p-4 hover:border-amber-300 dark:border-amber-500/35 hover:shadow-md transition-all">
                <button onClick={() => navigate(`${basePath}/${v.vendorId}`)} className="w-full text-left">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3 min-w-0">
                      <div className="w-10 h-10 rounded-xl bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-300 flex items-center justify-center flex-shrink-0">
                        <Building2 size={18} />
                      </div>
                      <div className="min-w-0">
                    ...
                    <p className="text-lg font-bold text-amber-700 dark:text-amber-300 whitespace-nowrap">{formatCurrency(v.totalDebt)}</p>
                  </div>
                </button>
                {canCreateExpense && (
...
function VendorDebtDetailPage({ basePath, canCreateExpense }) {
  ...
  const STATUS_CFG = {
    NONE: { label: 'Chưa trả', cls: 'bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-300 border-red-200 dark:border-red-500/28' },
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-5">
      ...
      <div className="bg-gradient-to-r from-amber-50 dark:from-amber-500/10 to-amber-50/40 rounded-2xl p-4 flex items-center justify-between border border-amber-100 dark:border-amber-500/18">
        <div>
          <p className="text-xs text-muted font-medium">Công nợ còn lại</p>
          <p className="text-2xl font-bold text-amber-700 dark:text-amber-300 mt-0.5">{formatCurrency(totalRemaining)}</p>
        </div>
        <Wallet size={28} className="text-amber-600/40" />
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {history.map(h => {
                ...
                return (
                  <tr key={h.requestVendorId} className="border-b border-line-soft last:border-0">
                    <td className="px-4 py-3 font-mono text-xs font-bold text-gold">{h.requestCode}</td>
                    <td className="px-4 py-3 font-semibold text-ink">{formatCurrency(h.totalAmount)}</td>
                    <td className="px-4 py-3 text-emerald-600 dark:text-emerald-300">{formatCurrency(h.paidAmount)}</td>
                    <td className="px-4 py-3 font-bold text-amber-700 dark:text-amber-300">{formatCurrency(h.remaining)}</td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex text-xs font-semibold px-2 py-0.5 rounded-full border ${cfg.cls}`}>{cfg.label}</span>
                    </td>
                    <td className="px-4 py-3 text-xs text-muted">{fmtTs(h.debtSince)}</td>
                  </tr>
                );
              })}
            </tbody>
