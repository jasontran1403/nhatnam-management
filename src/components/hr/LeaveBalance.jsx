    ...
    <Modal open={open} onClose={onClose}
      ...
      {items == null ? (
        <div className="space-y-2">
          {[1, 2, 3].map(i => <div key={i} className="h-12 bg-[#FAF7F2] rounded-xl animate-pulse" />)}
        </div>
      ) : items.length === 0 ? (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {items.map(i => (
                  <tr key={i.id} className="border-b border-black/5 last:border-0 align-top">
                    ...
                    <td className="px-3 py-2.5 text-center text-[#8E8878]">{i.calendarDays ?? '—'}</td>
                    <td className="px-3 py-2.5 text-center font-semibold text-[#C9A84C]">
                      {fmtDays(i.paidLeaveDays)}
                    </td>
                    <td className="px-3 py-2.5 text-center text-[#8E8878]">
                      {fmtDays(i.unpaidLeaveDays)}
                    </td>
                    <td className="px-3 py-2.5 text-xs text-[#1C1C1E] max-w-[220px]">
                      {i.reason || '—'}
                      {i.decisionNote && (
                        <div className="text-[11px] text-[#8E8878] mt-0.5">Ghi chú: {i.decisionNote}</div>
                      )}
                    </td>
                    <td className="px-3 py-2.5 text-xs text-[#8E8878] whitespace-nowrap">
                      {i.decidedByName || '—'}
                    </td>
                  </tr>
                ))}
              ...
              <tfoot>
                <tr className="border-t border-black/10 font-semibold">
                  ...
                  <td className="px-3 py-2.5 text-center text-[#C9A84C]">{fmtDays(totalPaid)}</td>
                  <td className="px-3 py-2.5 text-center text-[#8E8878]">{fmtDays(totalUnpaid)}</td>
                  <td colSpan={2} />
                </tr>
              </tfoot>
            </table>
...
export default function LeaveBalanceCard({ userId, year, compact = false, title }) {
  ...
  if (loading && !balance) {
    return <div className={`bg-[#FAF7F2] rounded-2xl animate-pulse ${compact ? 'h-12' : 'h-24'}`} />;
  }
  ...
  const remainColor = remaining < 0 ? 'text-red-600'
    : remaining <= 2 ? 'text-amber-600' : 'text-[#1C1C1E]';

  const HistoryButton = (
    <button type="button" onClick={() => setHistoryOpen(true)}
      className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#C9A84C]
                 hover:text-[#B69842] whitespace-nowrap">
    ...
    return (
      <>
        <div className="flex items-center gap-3 flex-wrap text-xs">
          <span className="text-[#8E8878]">Phép {y}:</span>
          <span className="text-[#1C1C1E]">
            <b>{fmtDays(balance.entitledDays)}</b> ngày
          </span>
          <span className="text-[#8E8878]">
            đã nghỉ <b className="text-[#1C1C1E]">{fmtDays(balance.usedDays)}</b>
          </span>
      ...
      <div className="bg-white rounded-2xl border border-black/5 shadow-sm p-4">
        <div className="flex items-center justify-between gap-3 mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-semibold text-[#1C1C1E]">{title || `Ngày phép năm ${y}`}</p>
              <p className="text-[11px] text-[#8E8878]">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            { label: 'Được nghỉ', value: balance.entitledDays, cls: 'text-[#1C1C1E]' },
            { label: 'Đã nghỉ', value: balance.usedDays, cls: 'text-[#8E8878]' },
            { label: 'Còn lại', value: remaining, cls: remainColor },
          ].map(x => (
            <div key={x.label} className="rounded-xl bg-[#FAF7F2] px-3 py-2.5 text-center">
              <p className={`text-xl font-bold ${x.cls}`}>{fmtDays(x.value)}</p>
              <p className="text-[11px] text-[#8E8878] mt-0.5">{x.label}</p>
            </div>
          ))}
        </div>

        {balance.missingWorkStartDate && (
          <div className="flex items-start gap-1.5 mt-3 text-[11px] text-amber-700 bg-amber-50
                          border border-amber-200 rounded-xl px-2.5 py-2">
