...
const ACTION_CFG = {
  APPROVED:       { label: 'Duyệt phiếu',        icon: CheckCircle, cls: 'text-green-600' },
  REJECTED:       { label: 'Từ chối phiếu',      icon: XCircle,     cls: 'text-red-600' },
  REOPENED:       { label: 'Mở lại → Chờ duyệt', icon: RotateCcw,   cls: 'text-amber-600' },
  ITEMS_UPDATED:  { label: 'Sửa khoản chi',      icon: Pencil,      cls: 'text-[#C9A84C]' },
  REASON_UPDATED: { label: 'Sửa lý do chi',      icon: Pencil,      cls: 'text-[#C9A84C]' },
};
...
export default function ExpenseVoucherLogList({ voucherId, refreshKey }) {
  ...
  return (
    <div>
      ...
      {loading ? (
        <p className="text-xs text-[#8E8878]">Đang tải nhật ký...</p>
      ) : failed ? (
        <p className="text-xs text-[#8E8878]">Không tải được nhật ký</p>
      ) : logs.length === 0 ? (
        <p className="text-xs text-[#8E8878]">Chưa có thao tác nào được ghi nhận</p>
      ) : (
        <div className="space-y-1.5">
          {logs.map(l => {
            const cfg = ACTION_CFG[l.action] || { label: l.action, icon: History, cls: 'text-[#8E8878]' };
            const Icon = cfg.icon;
            return (
              <div key={l.id} className="flex items-start gap-2 text-xs">
                ...
                <div className="min-w-0">
                  ...
                  {l.note && <p className="text-[#8E8878] mt-0.5 break-words">{l.note}</p>}
                </div>
              </div>
            );
          })}
