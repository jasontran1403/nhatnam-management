...
export default function StatCard({ label, value, icon: Icon, changePercent, loading, accent = 'gold' }) {
  ...
  return (
    <div className="bg-surface rounded-2xl border border-hairline p-5 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="text-[11px] uppercase tracking-widest text-muted font-semibold truncate">{label}</p>
          <div className="mt-2">
            {loading ? (
              <div className="h-7 w-24 rounded bg-canvas animate-pulse" />
            ) : (
              <p className="text-2xl font-bold text-ink leading-tight truncate">{value}</p>
            )}
          </div>
          {showChange && !loading && (
            <div className={`mt-2 inline-flex items-center gap-1 text-xs font-medium ${up ? 'text-emerald-600 dark:text-emerald-300' : 'text-red-600 dark:text-red-300'}`}>
