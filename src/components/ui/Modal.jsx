...
export default function Modal({ open, onClose, title, children, size = 'md', footer }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className={`
        relative bg-surface w-full ${SIZE[size] || SIZE.md}
      ...
      `}>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-hairline flex-shrink-0">
          <h3 className="font-bold text-ink text-base" style={{ fontFamily: 'var(--font-display)' }}>
            {title}
          </h3>
        ...
        {/* Footer */}
        {footer && (
          <div className="px-5 py-3.5 border-t border-hairline flex-shrink-0 bg-canvas/60">
            {footer}
          </div>
        )}
      </div>
